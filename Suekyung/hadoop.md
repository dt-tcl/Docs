...
* 초기에는 HDFS(Hadoop File System)과 MapReduce 프레임워크로 시작되었으나, 다양한 데이터 저장, 실행엔진, 프로그래밍 및 데이터 처리와 같은 하둡 생태계 전반(Hadoop Ecosystem)을 포함하는 의미로 확장됨.
...
* 네임노드
  * 네임노드는 HDFS에서 Master 역할을 하며, 메타데이터 관리와 데이터노드의 관리를 담당.
